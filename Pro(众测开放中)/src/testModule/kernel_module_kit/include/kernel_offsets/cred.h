...
/***************************************************************************
 * 获取 cred 结构体中 uid\euid 字段的偏移量
 * 参数: offset     输出参数，返回 uid\euid 字段相对于 cred 起始的偏移量（字节）
 * 返回: OK 表示成功；其它值为错误码
 ***************************************************************************/
KModErr get_cred_uid_offset(uint32_t& offset);
KModErr get_cred_euid_offset(uint32_t& offset);

/***************************************************************************
 * 获取 cred 结构体中 cap_inheritable 字段的偏移量
 * 参数: offset     输出参数，返回 cap_inheritable 字段相对于 cred 起始的偏移量（字节）
 * 返回: OK 表示成功；其它值为错误码
 ***************************************************************************/
KModErr get_cred_cap_inheritable_offset(uint32_t& offset);
