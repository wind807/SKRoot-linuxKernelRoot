...
static KModErr exec_kernel_shellcode_no_result(std::function<KModErr(asmjit::a64::Assembler* a)> fn) {
    aarch64_asm_ctx asm_ctx = init_aarch64_asm();
    auto a = asm_ctx.assembler();
    kernel_module::arm64_module_asm_func_start(a);
    RETURN_IF_ERROR(fn(a));
    kernel_module::arm64_module_asm_func_end(a);
    uint64_t r;
    return kernel_module::execute_kernel_asm_func(aarch64_asm_to_bytes(a), r);
}
