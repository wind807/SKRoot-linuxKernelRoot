#include "skroot_box_parasite_elf_def.h"
#include <algorithm>
#include <fstream>
#include <limits>
#include <map>
...
std::vector<std::string> printNeededLibsResult;
...
void ElfFile<ElfFileParamNames>::printNeededLibs() const {
    ...
    for (; rdi(dyn->d_tag) != DT_NULL; dyn++) {
        if (rdi(dyn->d_tag) == DT_NEEDED) {
            ...
            printNeededLibsResult.push_back(name);
            //printf("%s\n", name);
        }
    }
}
...
int parasite_check_so_link(const char* original_so_file_path, const char* implant_so_file_path) {
    try {
		...
		int patch = internal_parasite_patch_elf(sizeof(print_cmd) / sizeof(print_cmd[0]), print_cmd);
        if(patch) {
            return patch;
        }
        for(const auto & item : printNeededLibsResult) {
            if(item == implant_so_file_path) {
...
int parasite_start_link_so(const char* original_so_file_path, const char* implant_so_file_path)
{
    try {
