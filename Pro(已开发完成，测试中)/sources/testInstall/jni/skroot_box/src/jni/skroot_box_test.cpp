...
static SkBoxErr unsafe_test_root(const char* str_root_key, std::string& out) {
...
static SkBoxErr safe_test_root(const char* str_root_key, std::string& out) {
	fork_pipe_info finfo;
	if(fork_pipe_child_process(finfo)) {
        std::string info;
		SkBoxErr ret = unsafe_test_root(str_root_key, info);
...
const char* get_root_test_report(const char* str_root_key) {
    ...
    report.clear();
    std::string result;
    SkBoxErr err = safe_test_root(str_root_key, result);
    report = "get_root_proxy: " + to_string(err) + "\n\n" + result + "\n";
    return report.c_str();
}
