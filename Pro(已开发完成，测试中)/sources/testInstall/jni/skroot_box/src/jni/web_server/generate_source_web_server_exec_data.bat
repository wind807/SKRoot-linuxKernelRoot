...
set "work_path=%~dp0"
set "skroot_box_path=%~dp0../../../../skroot_box/src/jni"

echo %work_path%
echo %skroot_box_path%
...
echo.|"%skroot_box_path%/file_convert_to_source_tools/file_convert_to_source_tools.exe" %work_path%/libs/arm64-v8a/web_server
...
:: 将res.h文件中的文本进行替换
powershell -Command "(Get-Content res.h) -replace 'namespace {', 'namespace skroot_box {' | Set-Content res.h"
...
:: 将临时文件重命名为最终的文件名
move /Y res.h web_server_exec_data.h
...
echo Finished generating the 'web_server_exec_data.h' file!
