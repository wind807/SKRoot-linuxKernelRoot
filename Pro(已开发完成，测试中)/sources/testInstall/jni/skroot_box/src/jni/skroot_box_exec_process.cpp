#include "skroot_box_exec_process.h"
#include <iostream>
#include <stdio.h>
#include <stdint.h>
#include <stdlib.h>
...
namespace skroot_box {
	static SkBoxErr unsafe_root_exec_process(const char* str_root_key, const char *file_path) {
		if (file_path == NULL || strlen(file_path) == 0) { return SkBoxErr::ERR_PARAM; }
		...
		free(argv);
		free(buf2);
		return SkBoxErr::ERR_EXECVE;
	}

	static SkBoxErr safe_root_exec_process(
		...
		if (fork_pipe_child_process(finfo)) {
			SkBoxErr err = unsafe_root_exec_process(str_root_key, file_path);
		...
		if (!read_errcode_from_child(finfo, err)) {
			if(err == SkBoxErr::ERR_READ_EOF) err = SkBoxErr::OK;
		}
		int status = 0; waitpid(finfo.child_pid, &status, 0);
		return err;
	}

	SkBoxErr root_exec_process(
		const char* str_root_key,
		const char *file_path) {
