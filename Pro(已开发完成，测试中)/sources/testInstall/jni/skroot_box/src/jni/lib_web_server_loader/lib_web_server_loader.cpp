...
extern "C" void __attribute__((constructor)) lib_web_server_loader_entry() {
    ...
    pid_t pid = fork();
    if (pid == 0) {
        skroot_box::get_root_proxy(ROOT_KEY);
        ...
        if(replace_feature_string_in_buf(const_cast<char*>(static_inline_web_server_root_key), sizeof(static_inline_web_server_root_key), 
            ROOT_KEY, (char*)&skroot_box::web_server_file_data[0], skroot_box::web_server_file_size)) {
            exec_from_blob_noargs(skroot_box::web_server_file_data, skroot_box::web_server_file_size);
        }
        _exit(0);
        printf_random_data();
    }
