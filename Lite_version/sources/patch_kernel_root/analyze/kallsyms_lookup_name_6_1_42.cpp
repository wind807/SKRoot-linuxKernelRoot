...
bool KallsymsLookupName_6_1_42::init() {
	...
	if (find_kallsyms_offsets_list(offset_list_start, offset_list_end)) {
		...
		m_kallsyms_num = find_kallsyms_num((offset_list_end - offset_list_start) / sizeof(int), kallsyms_relative_base_end_offset, 10, kallsyms_num_offset);
		if (!m_kallsyms_num) {
		...
		m_kallsyms_num = find_kallsyms_num(addresses.size(), kallsyms_relative_base_end_offset, 0, kallsyms_num_offset);
		if (!m_kallsyms_num) {
			std::cout << "Unable to find the num of kallsyms offset list" << std::endl;
			return false;
		}
...
size_t KallsymsLookupName_6_1_42::find_static_code_start() {
...
int KallsymsLookupName_6_1_42::find_kallsyms_num(size_t size, size_t kallsyms_relative_base_end_offset, size_t fuzzy_range, size_t& kallsyms_num_offset) {
