...
KallsymsLookupName_6_4_0::KallsymsLookupName_6_4_0(const std::vector<char>& file_buf) : m_file_buf(file_buf)
{
}

KallsymsLookupName_6_4_0::~KallsymsLookupName_6_4_0()
{
}
...
bool KallsymsLookupName_6_4_0::init() {
	...
	if (find_kallsyms_offsets_list(offset_list_start, offset_list_end)) {
		...
		uint64_t ptr_table_start = addresses[0].first;
		uint64_t ptr_table_end = addresses[addresses.size() - 1].first;
		uint64_t offset_table_start = ptr_table_start - virtual_base;
		...
		maybe_kallsyms_num_offset = find_maybe_kallsyms_num2(addresses.size(), offset_table_start);
		if (!maybe_kallsyms_num_offset.size()) {
			std::cout << "Unable to find the num of kallsyms offset list" << std::endl;
			return false;
		}
...
bool KallsymsLookupName_6_4_0::find_kallsyms_addresses_list(std::vector<std::pair<uint64_t, uint64_t>>& addresses) {
...
bool KallsymsLookupName_6_4_0::find_kallsyms_offsets_list(size_t& start, size_t& end) {
	for (auto i = 60000; i > 30000; i -= 5000) {
...
size_t KallsymsLookupName_6_4_0::find_kallsyms_relative_base_offset(size_t offset_list_end) {
	for (int i = 0; i < 5; i++) {
...
std::vector<size_t> KallsymsLookupName_6_4_0::find_maybe_kallsyms_num2(size_t size, size_t addresses_list_start) {
	//Search upwards
	if (addresses_list_start < MAX_FIND_RANGE) {
		std::cout << "The starting position of kallsyms offset list is too small and abnormal." << std::endl;
		return {};
	}
	size_t maybe_num = size;

	// Go further and find 'kallsyms num'
	std::vector<size_t> maybe_kallsyms_num_offset;
	const int kallsyms_token_index_len = 256 * sizeof(short);
	for (size_t b = MAX_FIND_RANGE; b < addresses_list_start - kallsyms_token_index_len; b += sizeof(uint32_t)) {
		uint32_t num = *(uint32_t*)&m_file_buf[b];
		if (num == maybe_num) {
			maybe_kallsyms_num_offset.push_back(b);
		}
...
bool KallsymsLookupName_6_4_0::find_kallsyms_names_list(int kallsyms_num, size_t kallsyms_num_end_offset, size_t& name_list_start, size_t& name_list_end) {
	name_list_start = 0;
	name_list_end = 0;
...
bool KallsymsLookupName_6_4_0::find_kallsyms_markers_list(int kallsyms_num, size_t name_list_end_offset, size_t& markers_list_start, size_t& markers_list_end, bool & markers_list_is_align8) {
...
bool KallsymsLookupName_6_4_0::find_kallsyms_seqs_of_names_list(int kallsyms_num, size_t kallsyms_relative_base_end_offset, bool markers_list_is_align8, size_t& seqs_of_names_list_start, size_t& seqs_of_names_list_end) {
	/*
...
bool KallsymsLookupName_6_4_0::find_kallsyms_token_table(size_t kallsyms_markers_end_offset, size_t& kallsyms_token_table_start, size_t& kallsyms_token_table_end) {
...
bool KallsymsLookupName_6_4_0::find_kallsyms_token_index(size_t kallsyms_token_table_end, size_t& kallsyms_token_index_start) {
...
bool KallsymsLookupName_6_4_0::resolve_kallsyms_addresses_symbol_base(size_t code_static_start, uint64_t& base_address) {
...
bool KallsymsLookupName_6_4_0::resolve_kallsyms_offset_symbol_base(size_t code_static_start, int& base_off) {
	if (!has_kallsyms_symbol("_stext")) {
		return false;
	}
	if (has_kallsyms_symbol("_text")) {
 ...
 */
unsigned int KallsymsLookupName_6_4_0::kallsyms_expand_symbol(unsigned int off, char* result, size_t maxlen)
{
...
uint64_t KallsymsLookupName_6_4_0::kallsyms_sym_address(int idx) {
	if (!CONFIG_KALLSYMS_BASE_RELATIVE) {
...
uint64_t KallsymsLookupName_6_4_0::kallsyms_lookup_name(const char* name) {
...
bool KallsymsLookupName_6_4_0::has_kallsyms_symbol(const char* name) {
	std::unordered_map<std::string, uint64_t> syms = kallsyms_on_each_symbol();
	auto iter = syms.find(name);
