...
powershell -NoProfile -Command "$ins='#define LIBROOT_SERVER_DATA 1'; $old=Get-Content 'res.h'; @($ins)+$old | Set-Content -Encoding UTF8 'res.h'"
powershell -NoProfile -Command "$ins='#ifndef LIBROOT_SERVER_DATA'; $old=Get-Content 'res.h'; @($ins)+$old | Set-Content -Encoding UTF8 'res.h'"
...
:: 将临时文件重命名为最终的文件名
move /Y res.h lib_root_server_data.h
...
echo Finished generating the 'lib_root_server_data.h' file!
