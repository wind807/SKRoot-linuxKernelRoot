...
static ssize_t unsafe_create_su_hide_folder(const char* str_root_key) {
...
static ssize_t safe_create_su_hide_folder(const char* str_root_key) {
...
static ssize_t unsafe_del_su_hide_folder(const char* str_root_key) {
	ssize_t err = kernel_root::get_root(str_root_key);
	RETURN_ON_ERROR(err);
    std::string file_path = get_su_hide_folder_path_string(str_root_key);
	try {
...
static ssize_t safe_del_su_hide_folder(const char* str_root_key) {
	ssize_t err = ERR_NONE;
	fork_pipe_info finfo;
	if(fork_pipe_child_process(finfo)) {
