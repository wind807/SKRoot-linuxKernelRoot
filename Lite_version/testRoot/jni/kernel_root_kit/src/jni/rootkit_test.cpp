...
static KRootErr safe_test_root(const char* str_root_key, std::string& out) {
...
const char* get_root_test_report(const char* str_root_key) {
    ...
    report.clear();
    std::string result;
    KRootErr err = safe_test_root(str_root_key, result);
    report = "get_root:" + std::to_string(to_num(err)) + "\n\n" + result + "\n";
    return report.c_str();
}
