...
std::string handle_root_exec_process(const std::string & path) {
    ...
    std::stringstream sstr;
    sstr << "root_exec_processs err:" << to_num(err);
    return convert_2_json(sstr.str());
}
...
std::string handle_install_su() {
    ...
    param["err"] = std::to_string(to_num(err));
...
std::string handle_uninstall_su() {
    ...
    std::stringstream sstr;
    sstr << "uninstall su err:" << to_num(err) << std::endl;
    if (err != KRootErr::ERR_NONE) {
        return convert_2_json(sstr.str());
    }
...
void inject_su_thread() {
    ...
    // 1.杀光所有历史进程
    std::set<pid_t> out;
    err = kernel_root::find_all_cmdline_process(ROOT_KEY, g_inject_su_info.getAppName().c_str(), out);
    g_inject_su_info.append_console_msg("find_all_cmdline_process err:" + std::to_string((ssize_t)err) + ", cnt:" + std::to_string(out.size()));
    if (err != KRootErr::ERR_NONE) {
	...
	err = kernel_root::wait_and_find_cmdline_process(
		ROOT_KEY, g_inject_su_info.getAppName().c_str(), 60 * 1000, pid);
    g_inject_su_info.append_console_msg("waiting for process creation err:" + std::to_string(to_num(err)));
    if (err != KRootErr::ERR_NONE) {
        g_inject_su_info.working = false;
        return;
    }
	err = kernel_root::inject_process_env64_PATH_wrapper(ROOT_KEY, pid, su_dir_path.c_str(), kernel_root::api_offset_read_only_read_file);
    g_inject_su_info.append_console_msg("inject su err:" + std::to_string(to_num(err)) + ", errmsg:" + strerror(errno));
...
std::string handle_get_precheck_app_file_list(const std::string & app_name) {
	...
	if (err != KRootErr::ERR_NONE) {
        errmsg << "find_all_cmdline_process err:" << to_num(err) << std::endl;
    ...
    std::map<std::string, kernel_root::app_dynlib_status> so_path_list;
	err = kernel_root::parasite_precheck_app(ROOT_KEY, app_name.c_str(), so_path_list);
    if (err != KRootErr::ERR_NONE) {
        errmsg << "parasite_precheck_app error:" << to_num(err) << std::endl;
		if(err == KRootErr::ERR_EXIST_32BIT) {
            errmsg << "此目标APP为32位应用，无法寄生" << to_num(err) << std::endl;
		}
        return convert_2_json_m(errmsg.str());
	}
	if (!so_path_list.size()) {
        errmsg << "无法检测到目标APP的JNI环境，目标APP暂不可被寄生；您可重新运行目标APP后重试；或将APP进行手动加固(加壳)，因为加固(加壳)APP后，APP会被产生JNI环境，方可寄生！" << to_num(err) << std::endl;
		return convert_2_json_m(errmsg.str());
	}
	
	std::vector<std::tuple<std::string, kernel_root::app_dynlib_status>> sort_printf;
	for (const auto& item : so_path_list) {
		if(item.second != kernel_root::app_dynlib_status_running) {
			continue;
		}
		sort_printf.push_back({item.first, item.second});
	}
	for (const auto& item : so_path_list) {
		if(item.second != kernel_root::app_dynlib_status_not_running) {
			continue;
		}
		sort_printf.push_back({item.first, item.second});
	}
	for (const auto& item : sort_printf) {
		...
		auto app_dynlib_status = std::get<1>(item);
		std::filesystem::path filePath(file_path);
		std::string status = app_dynlib_status == kernel_root::app_dynlib_status_running ? " (正在运行)" : " (未运行)";
		if(app_dynlib_status == kernel_root::app_dynlib_status_running) {
...
std::string handle_inject_su_in_forever_app(const std::string & app_name, const std::string & so_path) {
	...
	std::string su_file_path;
    KRootErr err = kernel_root::install_su(ROOT_KEY, su_file_path);
    std::string su_dir_path = err == KRootErr::ERR_NONE ? (std::filesystem::path(su_file_path).parent_path().string() + "/") : "";
	...
	err = kernel_root::parasite_implant_su_env(ROOT_KEY, app_name.c_str(), so_path.c_str(), su_dir_path.c_str());
	printf("parasite_implant_su_env err:%zd\n", err);
	if (err != KRootErr::ERR_NONE) {
        param["errcode"] = "-4";
        std::string msg = "parasite_implant_su_env err:" + std::to_string(to_num(err));
...
void handle_client(int client_socket) {
    ...
    close(client_socket);
    writeToLog("Server client thread close.");
}
...
int main(int argc, char* argv[]) {
    ...
    if (kernel_root::get_root(ROOT_KEY) != KRootErr::ERR_NONE) {
