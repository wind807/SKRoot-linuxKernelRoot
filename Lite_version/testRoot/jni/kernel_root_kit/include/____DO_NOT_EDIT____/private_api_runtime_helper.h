...
inline KRootErr parasite_precheck_app(const char* str_root_key, const char* target_pid_cmdline, std::map<std::string, kernel_root::app_dynlib_status> &output_dynlib_full_path) {
    ...
    thread_local std::map<std::string, kernel_root::app_dynlib_status>* tls_out = nullptr;
    auto cb = [](const char* dynlib_full_path, kernel_root::app_dynlib_status status) {
        if (!tls_out || !dynlib_full_path) return;
        (*tls_out)[std::string(dynlib_full_path)] = status;
    };
    tls_out = &output_dynlib_full_path;
    KRootErr parasite_precheck_app_with_cb(const char* str_root_key, const char* target_pid_cmdline, void (*cb)(const char* dynlib_full_path, kernel_root::app_dynlib_status status));
