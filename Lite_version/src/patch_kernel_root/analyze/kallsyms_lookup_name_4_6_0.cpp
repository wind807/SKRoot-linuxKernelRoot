...
bool KallsymsLookupName_4_6_0::is_inited() const {
	return m_inited;
}
...
int KallsymsLookupName_4_6_0::find_kallsyms_num_by_names_logic(size_t start, int min_cnt, int max_cnt, size_t& kallsyms_num_offset) {
...
bool KallsymsLookupName_4_6_0::find_kallsyms_names_list(int kallsyms_num, size_t kallsyms_num_end_offset, size_t& name_list_start, size_t& name_list_end) {
	...
	if (!check_kallsyms_names_list_entropy(name_list_start, off, kallsyms_num)) {
		return false;
	}
	name_list_end = off;
	return true;
...
bool KallsymsLookupName_4_6_0::check_kallsyms_names_list_entropy(size_t start, size_t end, int kallsyms_num) {
	size_t total_parsed_size = end - start;
	// Global density verification (average size)
	// Even after high-frequency dictionary compression, the average length of normal kernel symbols is highly concentrated within the range of [1.5, 40.0] bytes.
	// If the total size is too unreasonable, just get rid of it.
	if (total_parsed_size < kallsyms_num * 1.5 || total_parsed_size > kallsyms_num * 40) {
		return false;
	}
	...
	// For a real compressed stream, over 200 types of bytes (0x00-0xFF) are typically utilized.
	// If the number of byte types used is less than 128, it must be a struct/constant array!
	if (unique_bytes_count < 128) {
		return false;
	}
	// Zero Ratio Check
	// In a genuine compressed stream, the proportion of 0x00 is extremely low (typically around 1% to 5%).
	// More than 15% of the bytes are 0x00, which is definitely not kallsyms_names.
...
uint64_t KallsymsLookupName_4_6_0::kallsyms_symbol_size(uint64_t cur_addr) {
